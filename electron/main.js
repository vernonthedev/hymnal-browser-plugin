...
class HymnBroadcastServer {
    ...
    async buildHymnIndex(hymnsDir) {
        ...
        try {
            ...
            const index = await Promise.all(hymnFiles.map(async (file) => {
...
async function getLatestReleaseInfo() {
    ...
    try {
        ...
        const nextHeaderIndex = lines.findIndex((line, index) => index > headerIndex && /^#\s*\[?\d+\.\d+\.\d+\]?/.test(line));
...
async function seedHymnsDir(targetHymnsDir) {
    ...
    for (const fileName of sourceFiles) {
        ...
        await fs.promises.copyFile(path.join(sourceDir, fileName), path.join(targetHymnsDir, fileName));
    }
}
