...
(() => {
  ...
  function renderLogs() {
    ...
    elements.logOutput.textContent = state.logLines.length ? state.logLines.join("\n") : "Waiting for backend logs...";
  }
    ...
    <div class="modal-copy">
      ...
      <div class="modal-list">
        ...
        <article class="modal-card">
          ...
          ${releaseSummary.length ? `<ul class="modal-bullets">${releaseSummary.map((item) => `<li>${item}</li>`).join("")}</ul>` : "<p>No changelog summary available.</p>"}
        </article>
      </div>
    </div>
  `;
  ...
  function getMatchingHymns(query) {
    ...
    return state.hymnIndex.filter((item) => {
  ...
  function renderFinderSpotlight() {
    ...
    if (!selectedHymn) {
      ...
      elements.finderSelectionPreview.textContent = "Type a number to pull matching hymns into the quick rail.";
      return;
    }
  ...
  function bindEvents() {
    ...
    elements.blankBtn.addEventListener(
      "click",
      () => sendCommand({ cmd: "blank" })
    );
    elements.showBtn.addEventListener(
      "click",
      () => sendCommand({ cmd: "show" })
    );
    elements.retriggerBtn.addEventListener(
      "click",
      () => sendCommand({ cmd: "retrigger" })
    );
    elements.reloadIndexBtn.addEventListener(
      "click",
    ...
    document.addEventListener("click", (event) => {
      ...
      if (elements.hymnSearchPopover && target instanceof Node && !elements.hymnSearchPopover.contains(target) && !elements.hymnInput.contains(target)) {
        state.pickerDismissed = true;
        elements.hymnSearchPopover.classList.add("is-hidden");
      }
    });
