...
function getSelectedHymn() {
    ...
    if (typed) {
        const exactMatch = state.hymnIndex.find((item) => normalizeText(item.number) === typed ||
    ...
    if (state.status?.current_hymn) {
        return state.hymnIndex.find((item) => normalizeText(item.number) === normalizeText(state.status?.current_hymn)) || null;
    }
    return null;
}
